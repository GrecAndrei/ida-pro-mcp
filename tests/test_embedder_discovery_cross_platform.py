...
def clean_state(monkeypatch):
    ...
    monkeypatch.setattr(
        "ida_pro_mcp.host.intelligence_core._MODEL_PATH_CACHE", None
    )
    return monkeypatch
...
def sandbox_home(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("ida_pro_mcp.host.intelligence_core.Path.home", classmethod(lambda cls: tmp_path))
    if IS_WINDOWS:
...
def test_select_state_path_string():
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("ida_pro_mcp.host.intelligence_core.os.path.isfile", lambda p: True)
...
def test_select_state_path_list_picks_existing():
    with pytest.MonkeyPatch.context() as mp:
        ...
        mp.setattr("ida_pro_mcp.host.intelligence_core.os.path.isfile", fake_isfile)
