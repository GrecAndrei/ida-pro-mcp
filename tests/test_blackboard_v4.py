...
def test_knowledge_resource_summary():
    ...
    result = resolver.read("ida://knowledge")
    assert result is not None
    data = json.loads(result["text"])
    assert "summary" in data
    assert data["summary"]["systems"] == 1
    assert data["summary"]["gaps_open"] == 1


def test_knowledge_resource_systems():
    db = tempfile.mktemp(suffix=".db")
    kg = KnowledgeGraph(db_path=db)
    kg.add_system("Crypto", members=["0x401000", "0x402000"])
    kg.add_system("Network", members=["0x403000"])

    resolver = _resolver(bb_path=db)
    result = resolver.read("ida://knowledge/systems")
    data = json.loads(result["text"])
    assert len(data["systems"]) == 2


def test_knowledge_resource_gaps():
    db = tempfile.mktemp(suffix=".db")
    kg = KnowledgeGraph(db_path=db)
    gid = kg.add_gap("WPA key derivation", priority=0.9)
    kg.fill_gap(gid, "0x401234")
    kg.add_gap("Beacon parser", priority=0.8)

    resolver = _resolver(bb_path=db)
    result = resolver.read("ida://knowledge/gaps")
    data = json.loads(result["text"])
    assert len(data["open"]) == 1
    assert len(data["filled"]) == 1


def test_knowledge_resource_attack_surface():
    db = tempfile.mktemp(suffix=".db")
    kg = KnowledgeGraph(db_path=db)
    kg.add_attack_surface("0x401000", reachable_from="air_unauthenticated")

    resolver = _resolver(bb_path=db)
    result = resolver.read("ida://knowledge/attack_surface")
    data = json.loads(result["text"])
    assert len(data["attack_surface"]) == 1


def test_knowledge_resource_peripherals():
    db = tempfile.mktemp(suffix=".db")
    kg = KnowledgeGraph(db_path=db)
    kg.add_peripheral("0xA0010000", name="AES", periph_type="crypto")

    resolver = _resolver(bb_path=db)
    result = resolver.read("ida://knowledge/peripherals")
    data = json.loads(result["text"])
    assert len(data["peripherals"]) == 1
    assert data["peripherals"][0]["periph_type"] == "crypto"


def test_knowledge_resource_state_machines():
    db = tempfile.mktemp(suffix=".db")
    kg = KnowledgeGraph(db_path=db)
    kg.add_state_machine("Auth SM", state_var="0x80420000")

    resolver = _resolver(bb_path=db)
    result = resolver.read("ida://knowledge/state_machines")
    data = json.loads(result["text"])
    assert len(data["state_machines"]) == 1
...
def test_state_returns_narrative_when_available():
    db = tempfile.mktemp(suffix=".db")
    bb = BlackboardStore(db_path=db)
    bb.write("Analysis Narrative",
             content="## Binary: test.bin\n\nThis is a WiFi firmware.\n\n## Understanding\nCoverage: 50%",
             category="narrative", confidence=1.0, embed=False)

    _res_mod.invalidate_state_cache()
    resolver = _resolver(bb_path=db)
    result = resolver.read("ida://state")
    assert result is not None
    # Should return text/plain with narrative
    assert result.get("mimeType") == "text/plain"
    assert "WiFi firmware" in result["text"]
    assert "<!-- state:" in result["text"]  # machine-readable header
