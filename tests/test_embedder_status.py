...
def test_embedder_falls_back_without_model_or_server(monkeypatch):
    ...
    monkeypatch.setattr("ida_pro_mcp.host.intelligence_core._find_llama_server", lambda: "")
    monkeypatch.setattr("ida_pro_mcp.host.intelligence_core._find_model", lambda: "")
    try:
        emb = BgeCodeEmbedder()
...
def test_embedder_respects_env_disable(monkeypatch):
    old = _reset_singleton()
    monkeypatch.setattr("ida_pro_mcp.host.intelligence_core.EMBED_DISABLED", True)
    monkeypatch.setattr("ida_pro_mcp.host.intelligence_core._find_llama_server", lambda: "/bin/echo")
    monkeypatch.setattr("ida_pro_mcp.host.intelligence_core._find_model", lambda: "/tmp/model.gguf")
    try:
        emb = BgeCodeEmbedder()
        assert emb.backend == "tfidf-fallback"
...
def test_status_has_fingerprints(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("ida_pro_mcp.host.intelligence_core._find_llama_server", lambda: str(server))
    monkeypatch.setattr("ida_pro_mcp.host.intelligence_core._find_model", lambda: str(model))
    try:
        emb = BgeCodeEmbedder()
