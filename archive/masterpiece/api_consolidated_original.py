    ...
    try:
        ...
        for addr in addrs:
            ...
            if action == "decompile":
                ...
                info = {"ok": True, "addr": addr, "name": fname, "size": hex_size(func.end_ea - func.start_ea)}
                ...
                info["callees"] = [{"addr": a, "name": n} for a, n in callees]
                ...
                info["callers"] = [{"addr": a, "name": n} for a, n in callers]
                        ...
                        for xref in idautils.XrefsFrom(item_ea, 0):
                            if xref.iscode:
                                ...
                                if tf and tf.start_ea != curr_ea:
                                    ...
                                    edges.append({"caller": hex_ea(curr_ea), "callee": hex_ea(target_ea)})
                                    if target_ea not in visited:
                                        visited[target_ea] = dist + 1
                                        queue.append((target_ea, dist + 1))
                
                results.append({"ok": True, "addr": addr, "graph": {hex_ea(k): v for k, v in visited.items()}, "edges": edges[:max_items]})
        ...
        if action == "functions":
                            ...
                            if len(s) >= 8:
                                ...
                                if alnum_count / len(s) >= 0.7:  # At least 70% meaningful
                                    if not query or query.lower() in s.lower():
                                        strings.append({"addr": hex_ea(sc.ea), "string": s[:200], "length": sc.length})  # Truncate long strings
                    except:
                        pass
            total = len(strings)
            ...
            for i in range(ida_nalt.get_import_module_qty()):
                ...
                def cb(ea, name, ordinal):
                    imports.append({"addr": hex_ea(ea), "name": name or f"ord_{ordinal}", "module": module})
                    return True
...
def types(
    ...
    try:
        if action == "list":
            ...
            # hti_flags: 0 = silent, default
            res, messages = parse_decls_ctypes(decl, 0)
            if res == 0:
                return {"ok": True}
            return {"error": f"Failed to declare types. Error code: {res}", "details": messages}
