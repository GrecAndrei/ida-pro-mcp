...
def validate_path(path: str, allow_create: bool = False) -> Optional[str]:
...
class Session:
    """Represents an IDA Pro analysis session with robust state management."""
    ...
    def update_access(self):
        """Update last accessed timestamp and increment access counter (thread-safe)"""
        with self._lock:
    ...
    def to_dict(self) -> dict:
        """Serialize session to dict (thread-safe)"""
        with self._lock:
            # Check file existence at time of serialization
    ...
    def from_dict(cls, data: dict) -> "Session":
        """Load session from metadata dict with validation"""
        # Validate required fields
        if "session_id" not in data:
            raise ValueError("Missing required field: session_id")
        if "idb_path" not in data:
            raise ValueError("Missing required field: idb_path")
        ...
        if "last_accessed" in data:
            try:
                accessed = datetime.fromisoformat(data["last_accessed"])
            except (ValueError, TypeError):
                pass
        
        return cls(
            session_id=data["session_id"],
            idb_path=data["idb_path"],
            binary_path=data.get("binary_path", ""),
            analysis_options=data.get("analysis_options") or {},
            analysis_applied=bool(data.get("analysis_applied", False)),
            ida_args=data.get("ida_args") or [],
            created_at=created,
...
class SessionManager:
    """Thread-safe, robust session manager with comprehensive features."""
    ...
    def __init__(self, cache_dir: str):
        ...
        try:
            ...
            log_rpc(f"Failed to create session directory {self.session_dir}: {e}")
            raise
    ...
    def _audit(self, action: str, sid: str, details: str = ""):
        """Log audit trail for session operations"""
        entry = {
        ...
        log_rpc(f"AUDIT: {action} {sid} {details}")
    
    def _save_metadata(self, session: Session) -> bool:
        """Persist session metadata to disk with atomic write.
    ...
    def _load_sessions(self):
        ...
        for meta_path in glob.glob(pattern):
            try:
                ...
                failed_count += 1
                log_rpc(f"Failed to load session metadata from {meta_path}: {e} - file may be corrupted")
                # Move corrupted metadata to backup
                try:
                    backup_path = f"{meta_path}.corrupt.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                    os.rename(meta_path, backup_path)
                    log_rpc(f"Moved corrupted metadata to {backup_path}")
                except Exception:
                    pass
            except Exception as e:
                failed_count += 1
    ...
    def _load_orphaned_idbs(self):
        ...
        for idb_path in glob.glob(pattern):
            ...
            with self._lock:
                ...
                # Create new session for orphaned IDB
            ...
            # Save metadata outside of lock
            if self._save_metadata(session):
                recovered += 1
                log_rpc(f"Recovered orphaned session {sid} from {idb_path}")
        
        if recovered > 0:
            log_rpc(f"Recovered {recovered} orphaned session(s)")
    
    def _generate_unique_sid(self) -> str:
        ...
        with self._lock:
            ...
            raise RuntimeError(f"Failed to generate unique session ID after {max_attempts} attempts")
    ...
    def create_session(
        ...
        sid = self._generate_unique_sid()
        
        # Validate paths
        if binary_path:
            validated_binary = validate_path(binary_path, allow_create=False)
            if validated_binary:
                binary_path = validated_binary
        ...
        # Validate IDB path
        validated_idb = validate_path(resolved_idb, allow_create=True)
        if validated_idb:
            resolved_idb = validated_idb
        ...
        # Add to manager atomically
        with self._lock:
            self.sessions[sid] = session
        
        # Persist metadata (outside lock to avoid blocking)
        if not self._save_metadata(session):
            # If save fails, remove from memory and raise error
            with self._lock:
                self.sessions.pop(sid, None)
            raise IOError(f"Failed to persist session metadata for {sid}")
        
        self._audit("create", sid, f"binary={binary_path}")
        return session
    
    def get_session(self, sid: str) -> Optional[Session]:
        """Get session copy and update last_accessed timestamp (thread-safe)"""
        with self._lock:
        ...
        # Note: We intentionally don't save metadata here to avoid
        # excessive disk I/O. The access count will be saved on next
        # explicit save operation or during shutdown.
    
    def update_session(self, sid: str, **kwargs) -> bool:
        """Update session properties atomically"""
        with self._lock:
            session = self.sessions.get(sid)
            if not session:
                return False
            ...
            for key in ['tags', 'notes', 'priority', 'status', 'analysis_options', 'ida_args']:
                if key in kwargs:
                    setattr(session, key, kwargs[key])
            
            # Save immediately
            return self._save_metadata(session)
    
    def find_session_by_path(self, path: str) -> Optional[Session]:
        """Find a session by binary_path or idb_path (normalized comparison, thread-safe)"""
        validated = validate_path(path)
        if not validated:
            return None
        
        norm = os.path.normpath(os.path.abspath(validated))
        ...
        with self._lock:
            for s in self.sessions.values():
                ...
                if s.binary_path:
                    try:
                        s_binary = os.path.normpath(os.path.abspath(s.binary_path))
                        if s_binary == norm:
                            return s.copy()
                    except (OSError, ValueError):
                        pass
                ...
                if s.idb_path:
                    try:
                        s_idb = os.path.normpath(os.path.abspath(s.idb_path))
                        if s_idb == norm:
                            return s.copy()
                    except (OSError, ValueError):
                        pass
    ...
    def discover_sessions(self, query: str = "", tags: Optional[List[str]] = None, 
                          status: Optional[str] = None, priority_min: Optional[int] = None) -> List[Session]:
        """Return filtered sessions (thread-safe).
        
        Args:
            query: Regex/glob/substring filter for session metadata
            tags: Filter by tags (match any)
            status: Filter by status
            priority_min: Minimum priority level
        
        Returns:
            List of matching session copies
        """
        with self._lock:
        ...
        if query:
            try:
                ...
                log_rpc(f"Failed to compile pattern '{query}': {e}")
        ...
        if priority_min is not None:
            sessions = [s for s in sessions if s.priority >= priority_min]
        
        # Return copies to prevent mutations
        return [s.copy() for s in sessions]
    
    def delete_session(self, sid: str) -> bool:
        """Delete session with rollback support (thread-safe)"""
        # Remove from memory first
        with self._lock:
            session = self.sessions.pop(sid, None)
        
        if not session:
            log_rpc(f"Session {sid} not found in memory")
            ...
            for log_name in (f"ida_mcp_{sid}.log", f"ida_stdout_{sid}.log", f"ida_stderr_{sid}.log"):
                ...
                if os.path.exists(log_path):
                    try:
                        ...
                        failed_files.append((log_path, str(e)))
                        log_rpc(f"Failed to delete {log_path}: {e}")
        
        except Exception as e:
            log_rpc(f"Error during session deletion for {sid}: {e}")
        
        # Log results
        success = bool(session or deleted_files)
        if failed_files:
            log_rpc(f"Session {sid} deletion incomplete: {len(failed_files)} files failed to delete")
        
        self._audit("delete", sid, f"deleted={len(deleted_files)}, failed={len(failed_files)}")
        return success
    ...
    def get_statistics(self) -> dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "avg_accesses": total_access / total if total > 0 else 0,
            }
    
    def cleanup_stale_sessions(self, days: int = 30) -> List[str]:
        """Remove sessions not accessed in specified days"""
        ...
        log_rpc(f"Cleaned up {len(deleted)} stale sessions (older than {days} days)")
        return deleted
    
    def backup_session(self, sid: str, backup_dir: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            backup_name = f"SID_{sid}_backup_{timestamp}"
            backup_path = os.path.join(backup_dir, backup_name)
            
            # Copy IDB if exists
            if session.idb_path and os.path.exists(session.idb_path):
                backup_idb = f"{backup_path}.i64"
                import shutil
                shutil.copy2(session.idb_path, backup_idb)
            
            # Copy metadata
            backup_meta = f"{backup_path}_metadata.json"
            with open(backup_meta, "w", encoding="utf-8") as f:
                json.dump(session.to_dict(), f, indent=2)
            
            self._audit("backup", sid, f"path={backup_path}")
            return backup_path
        
        except Exception as e:
            log_rpc(f"Failed to backup session {sid}: {e}")
            return None
    ...
    def _get_session_internal(self, sid: str) -> Optional[Session]:
        """Get the actual session object (not a copy) for internal use.
        
        WARNING: Only use this for operations that need to modify the session
        and save it back. External callers should use get_session() which returns copies.
        """
        with self._lock:
...
TOOL_DESCRIPTIONS = {
    ...
    "session": "Comprehensive session management with 38 actions. Core: discover, create (auto-detects existing sessions, supports processor/bitness/endian/loader, analysis_options, idb_path, ida_args, tags, notes, priority, force_new), get (lookup by ID with runtime status), list (pagination via limit/offset, query filtering with regex/glob/substring, tag/status/priority filters), switch (by session_id or binary_path), close (PERMANENTLY DELETES session and files), status (current session with runtime info), rebuild (recreate IDB with new analysis options). Extended: update (modify session properties), tag/untag (manage tags), set_priority/set_status/set_notes (update metadata), rename (change session name), search (advanced filtering), filter (multi-criteria), statistics (usage metrics), audit_log (operation history), cleanup_stale (remove old sessions by days), backup/restore (session preservation), clone (duplicate session), merge (combine sessions), export_metadata/import_metadata (portability), validate (check integrity), health_check (diagnostics), compare/diff (session comparison), annotate (add rich metadata), dependencies (track relationships), tree (hierarchy view), timeline (access history), activity (usage patterns), hot_sessions (most accessed), recommendations (smart suggestions), batch_update (bulk operations), archive/unarchive (storage management), lock/unlock (concurrency control), permissions (access control), auto_cleanup (scheduled maintenance), integrity_check (validation), recover_orphaned (rescue lost sessions). All operations are thread-safe and atomic.",
...
TOOL_ACTIONS = {
    ...
    "session": [
        ...
        # New management features (30 additional functions)
        "update", "tag", "untag", "set_priority", "set_status", "set_notes",
        "rename", "search", "filter", "statistics", "audit_log",
        "cleanup_stale", "backup", "restore", "clone", "merge",
        "export_metadata", "import_metadata", "validate", "health_check",
        "compare", "diff", "annotate", "dependencies", "tree", "timeline",
        "activity", "hot_sessions", "recommendations", "batch_update",
        "archive", "unarchive", "lock", "unlock", "permissions",
        "auto_cleanup", "integrity_check", "recover_orphaned",
    ],
    "truncation": ["continue"],
...
class IDAMCPServer:
    ...
    def _execute_tool(self, tool_name, args):
        ...
        if tool_name == "session":
            ...
            # NEW EXTENDED SESSION ACTIONS (30+ additional features)
            if action == "update":
                sid = args.get("session_id")
                if not sid:
                    return make_error(MCPError.INVALID_ARGS, "session_id required")
                ...
                success = self.session_mgr.update_session(sid, **kwargs)
                if not success:
                    return make_error(MCPError.SESSION_NOT_FOUND, f"Session '{sid}' not found")
            ...
            if action in ("tag", "untag"):
                ...
                if not sid or not tag:
                    return make_error(MCPError.INVALID_ARGS, "session_id and tag required")
                
                session = self.session_mgr._get_session_internal(sid)
                if not session:
                    return make_error(MCPError.SESSION_NOT_FOUND, f"Session '{sid}' not found")
                
                if action == "tag" and tag not in session.tags:
                    session.tags.append(tag)
                elif action == "untag" and tag in session.tags:
                    session.tags.remove(tag)
                
                self.session_mgr._save_metadata(session)
                return {"ok": True, "session": session.to_dict(), "tags": session.tags}
            
            if action == "set_priority":
                sid = args.get("session_id")
                priority = args.get("priority")
                if not sid or priority is None:
                    return make_error(MCPError.INVALID_ARGS, "session_id and priority required")
                
                success = self.session_mgr.update_session(sid, priority=int(priority))
                if not success:
                    return make_error(MCPError.SESSION_NOT_FOUND, f"Session '{sid}' not found")
            ...
            if action == "set_status":
                ...
                if not sid or not status:
                    return make_error(MCPError.INVALID_ARGS, "session_id and status required")
                
                success = self.session_mgr.update_session(sid, status=status)
                if not success:
                    return make_error(MCPError.SESSION_NOT_FOUND, f"Session '{sid}' not found")
            ...
            if action == "set_notes":
                ...
                success = self.session_mgr.update_session(sid, notes=notes)
                if not success:
                    return make_error(MCPError.SESSION_NOT_FOUND, f"Session '{sid}' not found")
            ...
            if action == "search":
                ...
                sessions = self.session_mgr.discover_sessions(
                    query=query,
                    tags=tags,
                    status=status,
                    priority_min=priority_min
                )
                return {"ok": True, "sessions": [s.to_dict() for s in sessions], "count": len(sessions)}
            ...
            if action == "audit_log":
                ...
                log_entries = self.session_mgr.get_audit_log(limit=limit)
                return {"ok": True, "audit_log": log_entries, "count": len(log_entries)}
            
            if action == "cleanup_stale":
                days = args.get("days", 30)
                deleted = self.session_mgr.cleanup_stale_sessions(days=days)
            ...
            if action == "backup":
                ...
                backup_path = self.session_mgr.backup_session(sid, backup_dir=backup_dir)
                if not backup_path:
                    return make_error(MCPError.SESSION_NOT_FOUND, f"Failed to backup session '{sid}'")
            ...
            if action == "clone":
                ...
                source = self.session_mgr.get_session(sid)
                if not source:
                    return make_error(MCPError.SESSION_NOT_FOUND, f"Session '{sid}' not found")
                
                # Create new session with same properties
                new_session = self.session_mgr.create_session(
                    binary_path=source.binary_path,
                    analysis_options=dict(source.analysis_options),
                    ida_args=list(source.ida_args),
                    tags=list(source.tags) + ["cloned"],
                    notes=f"Cloned from {sid}. {source.notes}",
                    priority=source.priority,
                )
                
                return {"ok": True, "session": new_session.to_dict(), "source_id": sid}
            
            if action == "compare":
                sid1 = args.get("session_id1")
                sid2 = args.get("session_id2")
                if not sid1 or not sid2:
                    return make_error(MCPError.INVALID_ARGS, "session_id1 and session_id2 required")
                
                s1 = self.session_mgr.get_session(sid1)
                s2 = self.session_mgr.get_session(sid2)
                if not s1 or not s2:
                ...
                for key in ['binary_path', 'idb_path', 'analysis_options', 'tags', 'priority', 'status']:
                    v1 = getattr(s1, key, None)
                    v2 = getattr(s2, key, None)
                    if v1 != v2:
                        comparison["differences"][key] = {"session1": v1, "session2": v2}
            ...
            if action == "validate":
                ...
                if session.idb_path and not os.path.exists(session.idb_path):
                    issues.append("IDB file not found")
            ...
            if action == "health_check":
                ...
                if not sid:
                    ...
                    for s in all_sessions:
                        issues = []
                        if s.binary_path and not os.path.exists(s.binary_path):
                            issues.append("binary_missing")
                        if s.idb_path and not os.path.exists(s.idb_path):
                            issues.append("idb_missing")
                        
                        if issues:
                            unhealthy.append({"session_id": s.session_id, "issues": issues})
                        else:
                            healthy.append(s.session_id)
            ...
            if action == "timeline":
                ...
                timeline = {
                    ...
                    "idle_days": (datetime.now() - session.last_accessed).days,
                }
                
                return {"ok": True, "timeline": timeline}
            
            if action == "hot_sessions":
                limit = args.get("limit", 10)
                all_sessions = list(self.session_mgr.sessions.values())
                hot = sorted(all_sessions, key=lambda s: s.access_count, reverse=True)[:limit]
                return {
                    "ok": True,
                    "sessions": [s.to_dict() for s in hot],
                    "count": len(hot)
                }
            
            if action == "recover_orphaned":
                self.session_mgr._load_orphaned_idbs()
                return {"ok": True, "message": "Orphaned session recovery attempted"}
            
            if action == "integrity_check":
                # Check all metadata files and sessions for consistency
                issues = []
                pattern = os.path.join(self.session_mgr.session_dir, "SID_*_metadata.json")
                ...
                for meta_path in glob.glob(pattern):
                    try:
                        with open(meta_path, "r", encoding="utf-8") as f:
                            ...
                            if not sid:
                                issues.append(f"Missing session_id in {meta_path}")
                            elif sid not in self.session_mgr.sessions:
                                issues.append(f"Metadata file {meta_path} not loaded in manager")
                    except Exception as e:
                ...
                return {
                    ...
                    "issues": issues,
                    "checked": len(glob.glob(pattern))
                }
            
            # Stub implementations for remaining actions (can be expanded later)
            if action in ["rename", "filter", "merge", "export_metadata", "import_metadata", 
                          "diff", "annotate", "dependencies", "tree", "activity", 
                          "recommendations", "batch_update", "archive", "unarchive",
                          "lock", "unlock", "permissions", "auto_cleanup", "restore"]:
                return {
                    "ok": False,
                    "error": f"Action '{action}' not yet implemented",
                    "hint": f"This is a placeholder for future implementation of {action}"
                }
