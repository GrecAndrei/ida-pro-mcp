"""Canonical crypto detection constants.

Crypto constants come from downloaded FindCrypt YARA rules — the YARA
scanner compiles and matches them directly. This module stores the
non-YARA constants used by ``search/core.py`` (CRYPTO_CONSTANT_NAMES) and
``code_helpers.py`` (DECOMP_CRYPTO_SIGS).
"""
...
CRYPTO_CONSTANT_NAMES: dict[int, str] = {
    ...
    # CRC32: 0x04C11DB7 is the normal (non-reflected) polynomial used by
    # ISO-HDLC/Ethernet; 0xEDB88320 is its bit-reversed (reflected) form used
    # by the common table-driven implementations.
    0x04C11DB7: "CRC32_POLY",
    0xEDB88320: "CRC32_POLY_REV",
    0x36E8E8E9: "CRC32",
    # Blowfish P-array
