...
# ============================================================
# SHARED CONSTANTS — defined once, used everywhere
# ============================================================
...
_CRYPTO_SIGNATURES = [
    ("AES S-Box",           "AES",       _AES_SBOX[:16],      "bytes"),
    ("AES S-Box (full)",    "AES",       _AES_SBOX,           "bytes"),
    ("AES Inverse S-Box",   "AES",       _AES_INV_SBOX_PREFIX, "bytes"),
    ("SHA-256 H values",    "SHA-256",   _SHA256_H[:4],       "dwords"),
    ("SHA-256 K constants", "SHA-256",   _SHA256_K[:4],       "dwords"),
    ("MD5 T constants",     "MD5",       _MD5_T[:4],          "dwords"),
    ("MD5 init values",     "MD5",       _MD5_INIT,           "dwords"),
    ("SHA-1 H values",      "SHA-1",     _SHA1_H[:4],         "dwords"),
    ("SHA-1 K constants",   "SHA-1",     _SHA1_K,             "dwords"),
    ("CRC32 table",         "CRC32",     _CRC32_TABLE_PREFIX[:4], "dwords"),
    ("Blowfish P-array",    "Blowfish",  _BLOWFISH_P[:4],     "dwords"),
    ("DES IP table",        "DES",       _DES_IP,             "bytes"),
    ("Base64 alphabet",     "Base64",    _BASE64_ALPHABET,    "bytes"),
    ("Base64 URL alphabet", "Base64-URL", _BASE64_URL_ALPHABET, "bytes"),
    ("Base32 alphabet",     "Base32",    _BASE32_ALPHABET,    "bytes"),
    ("ChaCha20 sigma",      "ChaCha20",  _CHACHA20_CONSTS,    "dwords"),
    ("Blake2b IV",          "Blake2",    _BLAKE2B_IV[:4],     "qwords"),
    ("SHA-3 RC",            "SHA-3/Keccak", _SHA3_RC[:2],     "qwords"),
]
...
_ANTI_DEBUG_APIS = [
    ...
    "NtQuerySystemInformation", "OutputDebugStringA", "OutputDebugStringW",
    "NtSetInformationThread", "NtClose", "UnhandledExceptionFilter", "NtQueryObject",
]
...
_HASH_RESOLVE_FUNCS = [
    "GetProcAddress", "GetProcAddressA", "LdrGetProcedureAddress", "LdrGetProcedureAddressEx",
]

_KNOWN_HASH_CONSTANTS = {
    "ror13_additive": 0x0D, "djb2": 0x1505, "sdbm": 0x1003F, "fnv1a_32": 0x811C9DC5,
}
...
def _scan_crypto_constants(scope_ea=None, limit=50) -> dict:
    ...
    # 1. Scan for known crypto constant byte sequences
    ...
    # 2. Encoding function names (base64, encode, decode)
    if len(findings) < limit:
    ...
    # 3. Checksum function names
    if len(findings) < limit:
        crc_hits = _search_dwords(_CRC32_TABLE_PREFIX[:4], limit - len(findings))
        for h in crc_hits:
            findings.append(f"{h}  algo=CRC32")
            algos.add("CRC32")
        adler_le = struct.pack("<H", _ADLER32_MOD)
        for h in _search_bytes(adler_le, limit - len(findings)):
            findings.append(f"{h}  algo=Adler32")
            algos.add("Adler32")
