...
TOOL_DESCRIPTIONS = {
    ...
    "firmware_view": "Raw firmware view-shaping assistant. Actions: scan_region, region_profile, segment_sweep, campaign, multi_region_campaign, campaign_checkpoint, campaign_resume, campaign_feedback, fingerprint_index_sync, fingerprint_index_query, pointer_sweep, pointer_clusters, table_candidates, carve_plan, auto_retype, smart_carve, recommend, rollback_last, review_contradictions. Integrates memrl + blackboard suggestions for iterative code/data reinterpretation and firmware triage planning. For raw blobs, start with binary_info(action='headers') and binary_info(action='sections') before retyping; for established firmware regions, the typical start is firmware_view(action='multi_region_campaign') followed by fingerprint sync/query for cross-image prioritization.",
...
TOOL_ACTIONS = {
    ...
    "firmware_view": ["scan_region", "region_profile", "segment_sweep", "campaign", "multi_region_campaign", "campaign_checkpoint", "campaign_resume", "campaign_feedback", "fingerprint_index_sync", "fingerprint_index_query", "auto_retype", "pointer_sweep", "pointer_clusters", "table_candidates", "carve_plan", "smart_carve", "recommend", "rollback_last", "review_contradictions"],
