...
TOOL_DESCRIPTIONS = {
    ...
    "annotation": "Automatically generates and manages comments, labels, and documentation across functions. Actions: auto_comment, label_loops, label_branches, mark_dangerous, annotate_constants, tag_functions, document_args, mark_error_paths, propagate_names, cleanup, validate.",
    ...
    "blackboard": "Firmware RE knowledge base: findings, hypotheses, IOCs, vulns, regions, and knowledge graph. Core: write, read, list, search, update, delete, clear, stats, prune, merge. Lifecycle: contradict, resolve, next_target. Evidence: add_evidence, calibrate. Campaign: campaign_summary, auto_tag_propagate. Crawler: start_crawler, stop_crawler, crawler_status, accept, reject. Proposals: accept_proposal, reject_proposal. KG write: add_system, add_struct, add_gap, fill_gap, add_state_machine, add_peripheral, add_attack_surface. KG read: kg_summary, kg_systems, kg_gaps, kg_structs, kg_state_machines, kg_attack_surface, kg_peripherals.",
    ...
    "calc": "Safe address arithmetic and pointer resolution—use instead of mental math. Actions: eval, offset, convert, resolve, deref, chain, align.",
    ...
    "classify": "Classifies functions and binaries by purpose (library, wrapper, callback, error handler, etc). Actions: function, binary, all_functions, library_code, wrappers, callbacks, initializers, error_handlers, hot_functions, orphans, induce_schema.",
    "code": "Core decompilation, disassembly, xrefs, and call graph queries for functions and addresses. Actions: decompile, disasm, xrefs_to, xrefs_from, xrefs_to_field, callees, callers, blocks, analyze, callgraph, export, find_paths, strings_in_func, diff_functions, semantic_decompile, decomp_dataflow, decompile_chain, json, c_header, prototypes.",
    ...
    "data": "Retrieve core IDB data: function lists, globals, strings, imports, exports. Actions: functions, globals, strings, imports, exports, lookup, bulk_query, capability_matrix.",
    ...
    "llm_helpers": "Context-optimized helpers for LLM agents analyzing binaries. Actions: context_window, function_digest, binary_digest, explain_address, suggest_next, progress_report, focus_area, question_answer, compact, enrich.",
    ...
    "memory": "Read, write, and inspect raw memory/bytes in the binary or debuggee. Actions: read, write, hexdump, search, compare, pointers, entropy, strings, struct_walk, histogram.",
    ...
    "patterns": "Generate, match, and manage FLIRT/byte pattern signatures for function identification. Actions: generate, match, list_sigs, apply_sig, create_sig, matched.",
    "predictor": "Deterministic prediction of next useful tool, focus address, or stuck-state detection. Actions: suggest_next_tool, detect_stuck, suggest_focus, suggest_next_address, risk_of_stall.",
    ...
    "search": "Search the IDB for patterns, bytes, text, and structured queries. Actions: text, bytes, regex, immediate, code_pattern, next, all, structured.",
    ...
    "session": "Full session lifecycle with runtime tracking, analysis notebook, hypothesis tracking, and skill crystallization. Actions: create/switch/close/list/status, snapshot/restore, crystallize_skill/rate_skill/suggest_strategy, notebook_append/read, track_hypothesis/confirm/refute, get_phase/advance_phase, recent_workset, macro_set/run, dashboard.",
