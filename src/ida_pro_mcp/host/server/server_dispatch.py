...
_SLICE_FORWARDABLE_TOOLS: dict[str, frozenset[str]] = {
    "data": frozenset({"functions", "globals", "strings", "imports", "exports", "annotations"}),
    "funcs": frozenset({"list"}),
}

# Actions that legitimately walk the IDB at scale (full-program scans,
# signature-index pipelines, decompile pumps, bulk-session housekeeping). For these we extend the socket recv
...
LONG_RUNNING_ACTIONS: set[tuple[str, str]] = {
    ...
    # intelligence — signature-index-heavy ops
    ...
    # search — full-binary scans, graph BFS, signature indexing
    ("search", "find"),
    ("search", "bytes"),
    ("search", "string"),
    ("search", "regex"),
...
def _long_running_sock_timeout(tool_name: str, rpc_args: dict) -> int:
    ...
    if tool_name == "intelligence" and (action == "index_batch" or (action in {"index_fast", "index_range"} and rpc_args.get("mode") == "full")):
        try:
    ...
    requested = rpc_args.get("timeout") or rpc_args.get("max_wait") or rpc_args.get("poll_timeout")
    try:
...
class ServerDispatchMixin(ServerClientStateMixin):
    ...
    @staticmethod
    def _policy_baseline_mode_cached(_key: tuple[int, int] | None, config_path: str) -> str:
    ...
    def _safe_mode_gate(self, sid: str, tool_name: str, action: str) -> dict | None:
        ...
        if tool_name in SAFE_MODE_BLOCKED_TOOLS:
            return make_error(
                ...
                recoverable=True,
                hint=("This operation can run arbitrary code or trigger full analysis of a database that is still being analyzed. Poll ida_session_status until safe_mode clears, then retry."),
    ...
    def call_tool(self, tool_name, idb_path, **kwargs):
        ...
        safe_gate = self._safe_mode_gate(_sid, tool_name, str(kwargs.get("action") or ""))
        ...
        try:
            ...
            if is_error_result(rpc_args):
                return rpc_args
            if (tool_name == "intelligence" and rpc_args.get("action") == "semantic_search") or (tool_name == "search" and rpc_args.get("action") == "nl"):
            ...
            try:
                _wallclock_cap = float(os.environ.get("IDA_MCP_RPC_HARD_WALLCLOCK_SEC", "900"))
            except Exception:
                _wallclock_cap = 900.0
            _wallclock_cap = max(_wallclock_cap, 30.0)
            _rpc_started = time.time()
            _sid = session.session_id
            # Attribute provider-backed advisory ranking to the IDB session
            # without exposing a new public argument or allowing the client to
            ...
            try:
                res = self._send_rpc_with_retry(
                    {"tool": tool_name, "args": rpc_args},
                    port,
                    recv_timeout=_rpc_sock_timeout,
                    queue_timeout=(None if RPC_QUEUE_TIMEOUT_SECONDS <= 0 else RPC_QUEUE_TIMEOUT_SECONDS),
                )
                ...
                return make_error(
                    ...
                    recoverable=True,
                    hint=("Wait for the in-flight call to finish, or reduce parallel calls to this session. Increase IDA_MCP_RPC_QUEUE_TIMEOUT (seconds) for deeper queues."),
                    details={
                        "session_id": _sid,
            ...
            if _elapsed_wallclock >= _wallclock_cap and is_error_result(res):
                try:
                    ...
                    logging.getLogger(__name__).debug("wallclock watchdog term failed: %s", _kill_e)
                return make_error(
                    MCPError.IDA_TIMEOUT,
                    f"Tool call exceeded wall-clock cap of {_wallclock_cap:.0f}s (IDA_MCP_RPC_HARD_WALLCLOCK_SEC). The IDA process was terminated — the next call will re-spawn it.",
                    recoverable=True,
                    details={
                        "tool": tool_name,
            ...
            if _elapsed_wallclock >= _wallclock_cap:
                log_rpc(f"wallclock cap {_wallclock_cap:.0f}s exceeded for {tool_name} (elapsed={_elapsed_wallclock:.1f}s) but RPC returned successfully")
            ...
            if isinstance(res, dict) and tool_name == "misc" and str(rpc_args.get("action") or "") in {"python", "idc", "plugin_run"}:
                try:
            ...
            if _pp and has_post_process(_pp) and not _pp.get("next_token") and not (isinstance(res, dict) and res.get("_post_processed")) and not is_error_result(res):
                try:
                    if _pp.get("_forwarded_offset") is not None:
                        ...
                        _apply_pp = {k: v for k, v in _pp.items() if k != "offset"}
                    else:
                        _apply_pp = _pp
            ...
            if isinstance(e, (_socket.timeout, TimeoutError, OSError)):
                return make_error(
                    MCPError.IDA_TIMEOUT,
                    f"IDA did not respond within {_recv_to}s (IDA_MCP_RPC_TIMEOUT). The process is still alive; the call likely needs more time. Retry, or raise IDA_MCP_RPC_TIMEOUT.",
                    recoverable=True,
    ...
    def _handle_session_health(self, args: dict) -> dict:
        ...
        action_counts = {str(tool): len(list(actions or [])) for tool, actions in TOOL_ACTIONS.items()}
        max_actions_tool = ""
        max_actions_count = 0
        if action_counts:
        ...
        try:
            ...
            logging.getLogger(__name__).debug("session discovery failed during health: %s", _discover_e)
    ...
    def _handle_bookmarks(self, args: dict) -> dict:
        ...
        if action == "list":
            filters = {key: args.get(key) for key in ("category", "tag", "priority", "query") if args.get(key) is not None}
    ...
    def _handle_next_continuation(self, tool_name: str, token: str, pp_params: dict) -> dict:
        ...
        if cached_session_id or cached_owner_id:
            if (cached_session_id and current_session_id != cached_session_id) or (cached_owner_id and current_owner_id != cached_owner_id):
                return make_error(
                    MCPError.TRUNCATION_TOKEN_INVALID,
        ...
        if _gr_err is not None:
            return _gr_err
        _bb_phase_err = self._blackboard_and_phase_preflight(tool_name, base_args, _replay_ack)
        if _bb_phase_err is not None:
            return _bb_phase_err

        ip = base_args.pop("idb", self.current_session.idb_path if self.current_session else None)
        if not ip:
            return make_error(
                MCPError.SESSION_REQUIRED,
    ...
    def _cache_post_process_next(
        ...
        *,
        ...
        current_offset = _bounded_int(pp_params.get("offset"), 0, min_value=0, max_value=500_000)
        has_more = source_truncated or (page_size is not None and total > (current_offset + count))
        ...
        with self._next_cache_lock():
            self._next_cache[token] = {
                ...
                "action": base_args.get("action"),
                "args": {k: v for k, v in base_args.items() if k != "action"},
                "post_process": {k: v for k, v in pp_params.items() if k not in {"next_token", "_forwarded_offset"}},
    ...
    def _execute_tool(self, tool_name, args):
        ...
        if isinstance(args, dict) and args.get("idb"):
            try:
                ...
                _target_session = None
        _exec_sid = getattr(_target_session, "session_id", None) if _target_session is not None else (getattr(self.current_session, "session_id", None) if self.current_session else None)
        ...
        _rl_action = str(args.get("action", "")) if isinstance(args, dict) else ""
            ...
            try:
                if not (isinstance(result, dict) and result.get("_post_processed")):
                    if pp_params.get("_forwarded_offset") is not None:
                        _apply_pp = {k: v for k, v in pp_params.items() if k != "offset"}
                    else:
                        _apply_pp = pp_params
                ...
                if not isinstance(cache_args, dict):
                    cache_args = args
                result = self._cache_post_process_next(resolved_tool, cache_args, pp_params, result)
            except Exception as _pp_err:
                import logging
        ...
        if isinstance(result, dict):
            ...
            if isinstance(err, dict):
                guardrail_blocked = err.get("code") == MCPError.INVALID_ARGS and "guardrail" in str(err.get("message", "")).lower()
                ...
                if code == MCPError.INVALID_ARGS and "guardrail" in str(message or "").lower():
                    guardrail_blocked = True
                if code is not None or message is not None:
                    error_str = str({"code": code, "message": message})[:500]
                else:
                    error_str = str(err)[:500]
            elif result.get("ok") is False:
                if result.get("code") == MCPError.INVALID_ARGS and "guardrail" in str(result.get("message", "")).lower():
                    guardrail_blocked = True
                error_str = str(
                    {
                        "code": result.get("code"),
    ...
    def _guardrail_strict_gate(self, tool_name: str, args: dict) -> dict | None:
        ...
        if not ack and tool_name in risky_tools and (act in risky_actions or signal >= 2.0):
            return make_error(
                MCPError.INVALID_ARGS,
    ...
    def _blackboard_and_phase_preflight(self, tool_name: str, args: dict, risk_ack_passed: bool) -> dict | None:
        ...
        try:
            ...
            if policy_mode == "off":
                return None
            if tool_name != "blackboard" and not risk_ack_passed and hasattr(self, "_bb_policy_bump") and hasattr(self, "_bb_policy_check"):
        ...
        try:
            ...
            if tool_name != "blackboard" and not risk_ack_passed and policy_mode != "off" and hasattr(self, "_phase_preflight_for_tool"):
    ...
    def _handle_provider_intelligence_status(self, args: dict[str, Any]) -> dict[str, Any]:
        """Return provider/accounting metadata without requiring an IDA session."""
        action = str(args.get("action") or "").strip()
        try:
            return self._handle_provider_intelligence_status_inner(action, args)
    ...
    def _handle_provider_intelligence_status_inner(self, action: str, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if action == "usage_report":
            return ledger.report(session_id=session_id, limit=args.get("limit", 100))
        return {"ok": True, "usage": ledger.status(session_id=session_id)}
    ...
    def _execute_tool_inner(self, tool_name, original_tool_name, args):
        ...
        if self._pending_pp and not self._pending_pp.get("next_token"):
            ...
            if _forwardable_actions and args.get("action") in _forwardable_actions:
                ...
                if _lim_from_count:
                    _lim = _native_count
                _pp_has_size = _pp.get("limit") is not None or _pp.get("head") is not None
                _pure_slice = (
                    _off is not None
                ...
                if _pure_slice:
                    try:
                        ...
                        _off_i = _lim_i = None
                    if _off_i is not None and _lim_i is not None and _off_i >= 0 and _lim_i > 0:
                        args["offset"] = _off_i
                        args["count"] = _lim_i
        ...
        if next_token and isinstance(next_token, str) and next_token.strip():
            continuation = self._handle_next_continuation(tool_name, next_token.strip(), self._pending_pp)
        ...
        try:
            ...
            if policy_result.decision != PolicyDecision.ALLOW or policy_result.risk.value != "read" or policy_result.reasons or policy_result.flags:
                try:
                    self.audit.log(
                        tool=tool_name,
        ...
        safe_gate = self._safe_mode_gate(_gate_sid, tool_name, str(args.get("action") or ""))
        if safe_gate is not None:
            return safe_gate
        ...
        _bb_phase_err = self._blackboard_and_phase_preflight(tool_name, args, _risk_ack_passed)
        if _bb_phase_err is not None:
            return _bb_phase_err
        ...
        try:
            ...
            if sid_for_drift and ui and ui.is_running():
                ...
                for sig in signals:
                    if _stuck_loop_block and sig.get("type") == "LOOP" and sig.get("severity") == "warning":
                        err = make_error(
                            MCPError.STUCK_LOOP,
        ...
        if tool_name == "intelligence" and str(args.get("action") or "").strip() in {
            "intelligence_status",
            "embedder_status",
            "reranker_status",
            "usage_status",
        ...
        if tool_name == "intelligence" and str(args.get("action") or "") in {"index_fast", "index_batch", "index_range"}:
            validation_error = self._validate_semantic_index_scope(args)
            if validation_error:
                return validation_error
            if _coerce_bool(args.pop("_background", False), False):
                idb_ref = args.pop("idb", self.current_session.idb_path if self.current_session else None)
                if not idb_ref:
                    return make_error(
                        MCPError.SESSION_REQUIRED,
