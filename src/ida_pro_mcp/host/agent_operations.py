...
and the translation to the legacy dispatcher. The in-band help tool and
`tools/list` serve this data live; there is no static skill snapshot.
"""
...
AGENT_OPERATIONS: tuple[AgentOperation, ...] = (
    ...
    AgentOperation(
        name="ida_compare_functions",
        description=(
            "Compare decompiled functions from two IDA sessions, with optional normalization "
            "of function-name, autogenerated-address, and whitespace noise. Numeric constants "
            "are preserved so security-relevant bound and mask changes remain visible."
        ),
        category="code",
        input_schema=_schema(
            {
                "left_session": {"type": "string", "description": "Session ID for the baseline function."},
                "left_address": ADDRESS,
                "right_session": {"type": "string", "description": "Session ID for the candidate function."},
                "right_address": ADDRESS,
                "normalize": {"type": "boolean", "description": "Normalize IDA naming and whitespace noise; defaults to true."},
                "context_lines": {"type": "integer", "minimum": 0, "maximum": 20, "description": "Unified-diff context lines; defaults to 3."},
                "max_diff_lines": {"type": "integer", "minimum": 1, "maximum": 2000, "description": "Maximum diff lines returned; defaults to 400."},
            },
            ["left_session", "left_address", "right_session", "right_address"],
        ),
        example={
