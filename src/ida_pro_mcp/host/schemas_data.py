...
TOOL_DESCRIPTIONS = {
    ...
    "symbols": "Loads and manages debug symbols (PDB/DWARF) for the current binary. Actions: load_pdb, load_dwarf, status, apply, export, import_system_map (bounded System.map import with optional address delta).",
    "multi_session": "Multi-binary session groups — link IDA sessions for cross-binary import/export resolution, cross-session decompilation, single-function and whole-session diffing, and cross-binary xref queries. Actions: group_create, group_list, group_link, group_remove, cross_resolve, cross_decompile, cross_diff, session_diff, cross_xrefs, status.",
