...
- `ida_diff_sessions(left_session=..., right_session=...)` to discover and rank modified functions across an entire vulnerable/patched build pair
...
- `ida_diff_sessions(...)` to auto-match functions, rank changes, identify added/removed calls and constants, and report functions found on only one side
