...
## Comparing functions across sessions

Open the baseline and candidate binaries as separate sessions, identify the
corresponding function in each, then call
`ida_compare_functions(left_session=..., left_address=..., right_session=...,
right_address=...)`. The operation decompiles in both runtimes and returns a
bounded unified diff plus raw and normalized similarity scores. Normalization
removes function-name, IDA autogenerated address-suffix, and whitespace noise;
it deliberately preserves numeric constants so changed bounds, masks, and
