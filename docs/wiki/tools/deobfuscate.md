...
## Actions
- `detect` — detect obfuscation techniques using BehaviorClassifier + heuristics; params: `address` (optional, whole-binary if omitted)
...
## Notes
- `detect` uses BehaviorClassifier with obfuscation anchors (obfuscation_xor, stack_strings, api_hashing) plus anti_debug/anti_vm detection, falling back to heuristics when embeddings are unavailable.
